...
def index():
    ...
    cards = query_db('SELECT * FROM magickarten')
...
def karte_neu():
    if request.method == 'POST':
        ...
        Staerke = request.form['staerke']
        Farbe = request.form['farbe']
        Kosten = request.form['manakosten']
        Abwehr = request.form['abwehr']
        ...
        cur.execute('INSERT INTO magickarten(Name,Typ,Staerke,Farbe,Kosten,Abwehr,Seltenheit,Holo,Serie,Anzahl,Decks) VALUES (?,?,?,?,?,?,?,?,?,?,?)',(Name,Typ,Staerke,Farbe,Kosten,Abwehr,Seltenheit,Holo,Serie,Anzahl,Decks))
        get_db().commit()
        get_db().close()
        return request.form
    else:
